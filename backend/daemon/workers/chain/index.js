        ...
        subscription.on('data', async (result) => {
            try {
                ...
                if (tx && tx.from) {
                    ...
                    const escrowAddr = (process.env.ESCROW_WALLET_ADDRESS || '').toLowerCase()
                    const hotAddr = (process.env.WITHDRAW_FROM_WALLET || '').toLowerCase()
                    
                    if ((escrowAddr && fromAddr === escrowAddr) || (hotAddr && fromAddr === hotAddr)) {
                        console.log(`[SUB][${coin}] ignorando transferencia interna de ${fromAddr}`)
                        return
                    }
                }
